...
export default function EditParticipantModal({
  ...
  useEffect(() => {
    if (participant) {
      ...
      const knownRelationships = ["", "PARENT", "LEGAL_GUARDIAN", "SPOUSE_PARTNER", "CHILDREN", "SUPPORT_COORDINATOR", "NOMINEE", "OTHER", "MYSELF"];
      const rawRelationship = participant.relationshipToClient || "";
      const isCustomRelationship = rawRelationship && !knownRelationships.includes(rawRelationship);
      ...
      setFormData({
        ...
        relationshipToClient: isCustomRelationship ? "OTHER" : rawRelationship,
        fundingType: participant.fundingType || "",
        ndisDetails,
        conditions: participant.conditions || [],
        notes,
      });
      setOtherRelationshipText(isCustomRelationship ? rawRelationship : "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({
        ...
        relationshipToClient: formData.relationshipToClient === "OTHER"
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-3 border-2 border-gray-200 rounded-lg bg-white text-left font-poppins focus:border-indigo-500 focus:outline-none"
                    >
                      <span className={formData.conditions.length > 0 ? "text-gray-900 text-sm" : "text-gray-500 text-sm"}>
                        {formData.conditions.length > 0
                          ? `${formData.conditions.length} condition${formData.conditions.length > 1 ? "s" : ""} selected`
                          : "Select or type a condition..."}
                      </span>
                      <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform flex-shrink-0 ${isConditionOpen ? "rotate-180" : ""}`} />
                    </button>

                    {/* Dropdown panel */}
                    {isConditionOpen && (
                      <div className="absolute z-10 w-full mt-1 bg-white border-2 border-gray-200 rounded-lg shadow-lg">
                        {/* Search/type input inside the dropdown */}
                        <div className="p-2 border-b border-gray-100">
                          <div className="relative">
                            ...
                            <input
                              ...
                              autoFocus
                              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-md font-poppins text-sm focus:border-indigo-500 focus:outline-none"
                  ...
                  {/* Selected tags shown below the dropdown */}
                  {formData.conditions.length > 0 && (
