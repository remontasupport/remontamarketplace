/**
 * Police Check Upload API
 *
 * Handles National Police Check document uploads to Vercel Blob storage
 * Stores in verification_requirements table
 * Accepts PDFs and images for proof of submission
 *
 * POST /api/upload/police-check
 */
...
export async function POST(request: Request) {
  console.log("🎯 Police check upload API called");
  ...
  try {
    ...
    const blobPath = `police-checks/${session.user.id}/${timestamp}-${sanitizedFileName}`;
    ...
    console.log(`✅ Police check uploaded to blob:`, blob.url);

    // 6. Check if police check document already exists
    console.log("🔍 Checking for existing police check document...");
    ...
    if (existingDoc) {
      ...
      console.log(`🔄 Updating existing police check document`);
      ...
      console.log(`✨ Creating new police check document`);
      verificationReq = await authPrisma.verificationRequirement.create({
        data: {
          workerProfileId: workerProfile.id,
          requirementType: "police-check",
          requirementName: "National Police Check",
          isRequired: true,
          documentUrl: blob.url,
          documentUploadedAt: new Date(),
          status: "SUBMITTED",
    ...
    console.error("❌ Police check upload error:", error);
