...
export default function Step3WorkingWithChildren({
  ...
  const fetchUploadedDocument = async () => {
    try {
      ...
      console.error("Failed to fetch working with children document:", error);
    }
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      console.log("📤 Starting upload to /api/upload/working-with-children");
      ...
      console.log("✅ Working With Children Check uploaded successfully:", responseData.url);
    } catch (error: any) {
              ...
              {uploadedDocument && !isEditMode && (
                <div className="points-summary-card complete mt-4">
                  <p className="points-summary-message complete">
                    Thank you for submitting your Working With Children Check. Our team will carefully review and verify your document.
                  </p>
                </div>
              )}
            </div>
