...
export default function Step5InfectionControl({
  ...
  const fetchUploadedDocument = async () => {
    try {
      const response = await fetch("/api/worker/infection-control");
      if (response.ok) {
      ...
      console.error("Failed to fetch infection control document:", error);
    }
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      console.log("📤 Starting upload to /api/upload/infection-control");

      const formData = new FormData();
      formData.append("file", file);
      formData.append("documentType", "infection-control");
      ...
      const response = await fetch("/api/upload/infection-control", {
        method: "POST",
        body: formData,
      });
      ...
      console.log("✅ Infection Control Training uploaded successfully:", responseData.url);
    } catch (error: any) {
